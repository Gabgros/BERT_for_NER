...
class Trainer:
    ...
    def training_step(self, data_loader):
        ...
        for batch in tqdm(data_loader):
            ...
            # Training loss and count of tokens
            train_loss += loss.item()
            n_tks += batch.input_ids.numel()
    ...
    def get_loss(self, logits, lbs):
        ...
        # --- TODO: start of your code ---
        loss = self._loss(logits.permute(0, 2, 1), lbs)
        return loss
        # --- TODO: end of your code ---
    ...
    def evaluate(self, dataset, detailed=False):
        ...
        pred_lbs: list[list[str]] = [] #might keep None

        # TODO: Predicted labels for each sample in the dataset and stored in `pred_lbs`, a list of list of strings.
        # TODO: The string elements represent the entity labels, such as "O" or "B-PER".
        # --- TODO: start of your code ---
        for batch in tqdm(data_loader):
            batch.to(self._device)
            outputs = self._model(input_ids=batch.input_ids,
                                  attention_mask=batch.attention_mask,
                                  labels=batch.labels).logits

            for phrase in range(outputs.size()[0]):
                batch_pred = []

                for tok in range(outputs[phrase].shape[0]):
                    if batch.attention_mask[phrase, tok] != 0 and batch.labels[phrase, tok] >= 0:
                        pred_label = torch.argmax(outputs[phrase, tok]).item()
                        batch_pred.append(self._config.bio_label_types[pred_label])

                pred_lbs.append(batch_pred)
        # --- TODO: end of your code ---
