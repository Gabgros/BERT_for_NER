...
class Dataset(torch.utils.data.Dataset):
    ...
    def encode(self, tokenizer_name: str, lb2idx: dict):
        ...
        for i, (tokens, labels) in enumerate(zip(self._token_ids, self._lbs)):
            token_word_id = tokenized_text.word_ids(batch_index=i)
            token_labels = [MASKED_LB_ID]
            seen_words = set()
            lbl_idx = 0

            for j in range(1, len(tokens)-1):
                if token_word_id[j] not in seen_words:
                    seen_words.add(token_word_id[j])
                    token_labels.append(lb2idx[labels[lbl_idx]])
                    lbl_idx += 1

                else:
                    token_labels.append(MASKED_LB_ID)

            token_labels.append(MASKED_LB_ID)
            bert_lbs_list.append(token_labels)
